...
import MainDialog from 'components/infrastructure/network/nodes/MainDialog.vue'
...
const columns = [
  ...
  { name: 'auth_server', label: 'Servidor de Autenticación', align: 'left' },
  { name: 'nc', label: 'NC', align: 'left' },
  { name: 'state', label: 'Departamento', align: 'left' },
  { name: 'municipality', label: 'Municipio', align: 'left' },
  { name: 'district', label: 'Distrito', align: 'left' },
...
const showDeleteDialog = (id, name) => {
  ...
  deleteProps.value = {
    title: 'Eliminar Nodo',
    message: `¿Deseas eliminar el nodo ${name} de los registros?`,
    id: id,
    url: '/api/v1/infrastructure/network/nodes/',
  }
}
    ...
    <BaseDataTable :columns="columns">
      <template v-slot:body="{ props }">
        <q-tr :props="props">
          ...
          <!--    Auth Server     -->
          <q-td key="auth_server" class="text-left copy-text" :props="props">
            {{ props.row?.auth_server?.name }}
          </q-td>

          <!--    NC     -->
          <q-td key="nc" class="text-left copy-text" :props="props" @click="copy(props.row?.nc)">
            {{ props.row?.nc }}
          </q-td>
