...
import MaritalDialog from 'components/configuration/clients/MaritalDialog.vue'
...
const showDeleteDialog = (id, name) => {
  ...
  deleteProps.value = {
    title: 'Eliminar estado marital',
    message: `¿Deseas eliminar el estado ${name} de los registros?`,
    id: id,
    url: '/api/v1/configuration/clients/marital/',
  }
}
...
<template>
  <div>
    ...
    <template v-if="showForm !== 0">
      <BaseDialog :id="currentItem" :content="MaritalDialog" />
    </template>
