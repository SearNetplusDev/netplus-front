...
import StatusDialog from 'components/configuration/infrastructure/equipments/StatusDialog.vue'
...
const showDeleteDialog = (id, name) => {
  ...
  deleteProps.value = {
    ...
    message: `¿Deseas eliminar el estado ${name} de los registros?`,
    id: id,
    url: '/api/v1/configuration/infrastructure/equipments/status/',
  }
}
...
<template>
  <div>
    ...
    <template v-if="showForm !== 0">
      <BaseDialog :id="currentItem" :content="StatusDialog" />
    </template>
