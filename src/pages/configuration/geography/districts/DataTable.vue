...
const columns = [
  ...
  { name: 'name', label: 'Nombre', align: 'left' },
  {
    ...
    filterable: true,
    model: [],
    filterURL: '/api/v1/general/municipalities',
    options: [],
    align: 'left',
  },
  {
...
const showDeleteDialog = (id, name) => {
  ...
  deleteProps.value = {
    ...
    message: `¿Deseas eliminar el distrito ${name} de los registros?`,
    id: id,
    url: '/api/v1/configuration/geography/districts/',
  }
}
...
<template>
  <div>
    ...
    <template v-if="showForm !== 0">
      <BaseDialog :id="currentItem" :content="DistrictDialog" />
    </template>
      ...
      <template v-slot:body="{ props }">
        <q-tr :props="props">
          ...
          <q-td
            ...
            class="text-left copy-text"
            :props="props"
            @click="copy(props.row?.municipality?.name)"
          >
