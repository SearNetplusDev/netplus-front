...
import BranchDialog from 'components/configuration/branches/BranchDialog.vue'
...
const columns = [
  ...
  { name: 'mobile', label: 'Celular', align: 'left' },
  { name: 'address', label: 'Dirección', align: 'left' },
...
const showDeleteDialog = (id, name) => {
  ...
  deleteProps.value = {
    ...
    message: `¿Deseas eliminar ${name} de los registros?`,
    id: id,
    url: '/api/v1/configuration/branches',
  }
}
...
<template>
  <div>
    ...
    <template v-if="showForm !== 0">
      <BaseDialog :id="currentItem" :content="BranchDialog" />
    </template>
      ...
      <template v-slot:body="{ props }">
        <q-tr :props="props">
          ...
          <q-td
            ...
            class="text-left copy-text"
            :props="props"
            @click="copy(props.row?.municipality)"
          >
