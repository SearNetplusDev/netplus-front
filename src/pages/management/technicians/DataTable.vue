...
import TechniciansFormDialog from 'components/administracion/users/TechniciansFormDialog.vue'
...
const columns = [
  ...
  { name: 'phone', label: 'Teléfono', align: 'left' },
  { name: 'hiring_date', label: 'Fecha Contratación', align: 'left' },
...
const showDeleteDialog = (id, name) => {
  ...
  deleteProps.value = {
    ...
    id: id,
    url: '/api/v1/management/technicians/',
  }
}
...
<template>
  <div>
    ...
    <template v-if="showForm !== 0">
      <BaseDialog :id="currentItem" :content="TechniciansFormDialog" />
    </template>
      ...
      <template v-slot:body="{ props }">
        <q-tr :props="props">
          ...
          <q-td
            ...
            :props="props"
            @click="copy(props.row?.phone_number)"
          >
            {{ props.row?.phone_number }}
          </q-td>

          <!--    Hiring Date     -->
          <q-td key="hiring_date" class="text-left copy-text" :props="props">
            {{ props.row?.hiring_date }}
          </q-td>
            ...
            <q-btn-group>
              <q-btn color="primary" icon="edit" size="sm" @click="edit(props.row?.id)">
                <q-tooltip transition-show="fade" transition-hide="flip-left" class="bg-grey-10">
                  Editar info. de {{ props.row?.user?.name }}
                </q-tooltip>
              </q-btn>
              <q-btn
                color="negative"
                icon="delete_forever"
                size="sm"
                @click="showDeleteDialog(props.row?.id, props.row?.user?.name)"
              >
                <q-tooltip transition-show="fade" transition-hide="flip-left" class="bg-grey-10">
                  Eliminar a {{ props.row?.user?.name }}
                </q-tooltip>
              </q-btn>
            </q-btn-group>
          </q-td>
