...
import StatusFormDialog from 'components/supports/StatusFormDialog.vue'
...
const columns = [
  ...
  { name: 'badge', label: 'Color', align: 'left' },
...
<template>
  <div>
    <template v-if="showForm !== 0">
      <BaseDialog :id="currentItem" :content="StatusFormDialog" />
    </template>
    ...
    <BaseDataTable :columns="columns">
      <template v-slot:body="{ props }">
        <q-tr :props="props">
          ...
          <!--   Badge Color      -->
          <q-td key="badge" class="text-left copy-text" :props="props">
            <q-badge
              :label="props.row.badge_color"
              class="text-center text-weight-bold q-py-sm"
              :style="{ backgroundColor: props.row.badge_color, color: '#fff' }"
            />
          </q-td>
