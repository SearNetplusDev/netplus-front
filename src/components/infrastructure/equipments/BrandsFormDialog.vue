...
const url = 'api/v1/infrastructure/equipments/brands/'
...
const getData = () => {
  ...
  api
    .post(`${url}edit`, data)
    .then((res) => {
      let itm = res.data.brand
      fields.name.data = itm.name
      fields.status.data = itm.status_id
      title.value = `Editar datos de la marca: ${itm.name}`
    })
    .catch((err) => {
  ...
  api
    ...
    .then((res) => {
      if (res.data.saved) {
        ...
        title.value = 'Editar datos de la marca: ' + res.data.brand?.name
      } else {
...
onMounted(() => {
  if (props.id > 0) {
    ...
    title.value = 'Registrar nueva marca'
  }
})
</script>
              ...
              <div class="fit row">
                <div class="col-12">
                  <q-breadcrumbs class="text-white" active-color="white" gutter="md">
                    ...
                    <q-breadcrumbs-el label="Marcas" icon="mdi-tag-multiple-outline" />
                  </q-breadcrumbs>
                </div>
              </div>
            </q-card-section>
