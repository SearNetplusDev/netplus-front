...
const url = 'api/v1/configuration/clients/phones/'
...
const getData = () => {
  ...
  api
    ...
    .then((res) => {
      ...
      title.value = `Editar datos de: ${itm.name}`
    })
    .catch((err) => {
  ...
  api
    ...
    .then((res) => {
      if (res.data.saved) {
        ...
        title.value = `Editar datos de: ${res.data.type.name}`
      } else {
...
onMounted(() => {
  if (props.id > 0) {
    ...
    title.value = 'Registrar nuevo tipo de teléfono'
  }
})
</script>
              ...
              <div class="fit row">
                <div class="col-12">
                  <q-breadcrumbs class="text-white" active-color="white" gutter="md">
                    ...
                    <q-breadcrumbs-el label="Tipos de teléfono" icon="mdi-phone-settings" />
                  </q-breadcrumbs>
                </div>
              </div>
            </q-card-section>
