...
const url = 'api/v1/configuration/clients/documents/'
...
const getData = () => {
  ...
  api
    ...
    .then((res) => {
      ...
      title.value = `Editar datos del tipo de documento: ${itm.name}`
    })
    .catch((err) => {
...
onMounted(() => {
  if (props.id > 0) {
    ...
    title.value = 'Registrar tipo de documento'
  }
})
</script>
              ...
              <div class="fit row">
                <div class="col-12">
                  <q-breadcrumbs class="text-white" active-color="white" gutter="md">
                    ...
                    <q-breadcrumbs-el label="Tipos de documento" icon="badge" />
                  </q-breadcrumbs>
                </div>
              </div>
            </q-card-section>
